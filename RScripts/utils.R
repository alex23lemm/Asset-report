...
# Utility functions used during report generation ------------------------------


xlsx.writeMultipleData <- function(file, sheet_name, ...)
  # Writes several data frames to a single xlsx file
  #
  # Args:
  ...
  
{
  ...
  for (i in 1:nobjects) {
    if (i == 1)
      write.xlsx(objects[[i]], file, sheetName = sheet_name[i])
    else write.xlsx(objects[[i]], file, sheetName = sheet_name[i],
                    append = TRUE)
  }
}
